...
<head>
  ...
  <title>FeynML Diagnostic Report — {self.project_name}</title>
  <style>
...
<div class="wrapper">
  ...
  <h2>FeynML Diagnostic Investigation &nbsp;·&nbsp; {self.timestamp[:19].replace('T',' ')} UTC</h2>
  ...
  <p style='color:#555;margin-top:32px;font-size:0.8em'>
    FeynML &nbsp;·&nbsp; ML Failure Investigation Platform &nbsp;·&nbsp;
    Investigation ID: {findings.get("sections", {}).get("root_cause", {}).get("investigation_id", "—")}
    &nbsp;·&nbsp; {self.timestamp[:10]}
  </p>
</div>
</body>
